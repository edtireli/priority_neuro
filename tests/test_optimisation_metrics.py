...
def auth_token(client):
    ...
    db = TestingSessionLocal()
    token_val = (
        db.query(User).filter(User.email == "z@x.com").first().verification_token
    )
    db.close()
    client.post("/api/auth/verify", json={"token": token_val})
    login = client.post(
        "/api/auth/login", json={"email": "z@x.com", "password": "pass"}
    )
...
def create_job(client, headers):
    proj = client.post(
        "/api/projects", json={"name": "P", "description": ""}, headers=headers
    ).json()
    pid = proj["id"]
    cfg = {
    ...
    job = client.post(
        f"/api/projects/{pid}/jobs", data={"config": json.dumps(cfg)}, headers=headers
    ).json()
...
def test_results_api(client):
    ...
    assert all(
        "iteration" in m and "information_gain" in m and "se" in m
